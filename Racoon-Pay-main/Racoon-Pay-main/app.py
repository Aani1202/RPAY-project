...
def get_db_connection():
    ...
    if conn is None:
        conn = g._database = sqlite3.connect('userda.db')
    return conn
def getUser():
...
def login():
    ...
    cursor = conn.cursor() 
    cursor.execute("SELECT * FROM usersda WHERE username=? AND password=?", (username, password))
...
def uPayCon():
    ...
    cursor = conn.cursor() 
    tag = 'N'
    cursor.execute("SELECT * FROM usersda WHERE tag=?", (tag))
    ...
    lenaddress = len(receivername)
    print(type(lenaddress))
    return render_template('paycon.html',user=session.get("currentuser","sessionerror"),currentaddress=session.get("currentaddress","sessionerror"),receivername=receivername,receiveraddress=receiveraddress,lenaddress=lenaddress)
